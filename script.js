...
function initTerminalJourney() {
    ...
    const journeyLines = [
        ...
        { type: 'code', text: 'STA $D020       ; changed border color. felt like god.' },
        { type: 'empty' },
        { type: 'comment', text: '// C era - pointers pointing to pointers to pain' },
        ...
        { type: 'empty' },
        { type: 'comment', text: '# FreeBSD - where real hackers compiled kernels' },
        { type: 'code', text: 'make buildworld && make installworld' },
        { type: 'code', text: '# 6 hours later: "oh a typo in rc.conf"' },
        { type: 'empty' },
        { type: 'comment', text: '# Render farms & grids - 1000 machines was "cute"' },
        { type: 'code', text: 'for i in $(seq 1 1000); do' },
        { type: 'code', text: '    ssh node$i "nice -n 19 render frame_$i" &' },
        { type: 'code', text: 'done' },
        { type: 'code', text: '# works. until one node sneezes. electricity bill: yes.' },
        { type: 'empty' },
        { type: 'comment', text: '# Finance - a new chapter' },
        { type: 'code', text: '# ever since I was young I dreamed of transforming' },
        { type: 'code', text: '# unstructured data into actionable business insights' },
...
function initWaveVisualization() {
    ...
    function draw() {
        ...
        if (targetMouse.x !== null) {
            mouse.x = mouse.x === null ? targetMouse.x : mouse.x + (targetMouse.x - mouse.x) * 0.1;
            mouse.y = mouse.y === null ? targetMouse.y : mouse.y + (targetMouse.y - mouse.y) * 0.1;
        } else {
            mouse.x = null;
            mouse.y = null;
        }
...
function initScrollAnimations() {
    ...
    ScrollTrigger.create({
        ...
        onEnter: () => {
            ...
            gsap.to('.layer-1', { opacity: 0.1, scale: 1, duration: 0.8, ease: 'power2.out' });
            gsap.to('.layer-2', { opacity: 0.2, scale: 1, duration: 0.8, delay: 0.15, ease: 'power2.out' });
            gsap.to('.layer-3', { opacity: 0.4, scale: 1, duration: 0.8, delay: 0.3, ease: 'power2.out' });
            gsap.to('.layer-4', { opacity: 0.8, scale: 1, duration: 0.8, delay: 0.45, ease: 'power2.out' });
...
function initMatrixLocation() {
    ...
    const pairs = [
        { response: 'There\'s no place like 127.0.0.1', subtitle: 'localhost, sweet localhost' },
        { response: 'Successfully invisible', subtitle: 'VPN detected. Nice try.' },
        { response: 'ping reality: Request timed out', subtitle: 'packet loss: 100%' },
        { response: '/dev/null', subtitle: 'where all locations go to rest' },
        { response: 'Location: undefined', subtitle: 'Mental state: caffeinated' },
        { response: 'Result: NULL', subtitle: 'db.locations.find({entity: "nulloxide"}) → [ ]' },
        { response: 'Somewhere between 0 and 1', subtitle: 'lat: NaN, long: undefined' },
        { response: '404: Physical location not found', subtitle: 'coordinates: [consciousness, caffeine]' },
    ];
...
function initNicknameInput() {
    ...
    function processNickname() {
        ...
        if (match) {
            ...
            response.textContent = defaultResponses[Math.floor(Math.random() * defaultResponses.length)];
        }
...
// Console Easter Egg
// ================================
console.log('%c∅→ nulloxide', 'font-size: 20px; font-weight: bold; color: #00ff9d;');
console.log('%c' + `
    ╔══════════════════════════════════════╗
...
window.nulloxide = {
    ...
    age: () => {
        ...
        console.log(`%c${years} years, ${days} days, ${hours} hours`, 'color: #00ff9d;');
    ...
    source: () => {
        ...
        console.log('%cOr just scroll up and read the code comments.', 'color: #4a4a58;');
    }
};
...
function initEndSection() {
    ...
    if (timestampEl) {
        ...
        timestampEl.textContent = timestamps[Math.floor(Math.random() * timestamps.length)];
    }
